    ...
    <section className="theme-card rounded-[1.75rem] px-6 py-6 sm:px-8 sm:py-8">
      ...
      <ol className="space-y-3">
        {items.map((item) => (
          <li
            key={item.questionId}
            className="theme-subtle-surface rounded-3xl px-5 py-4"
          >
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                <p className="mt-2 text-sm leading-6 text-[color:var(--app-text)] sm:text-base">
                  {item.questionText}
                </p>
              </div>
              <span
                className={`inline-flex w-fit rounded-full px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] ${
                  item.status === "correct"
                    ...
                    : item.status === "wrong"
                      ...
                      : "theme-muted-surface text-[color:var(--app-text-muted)]"
                }`}
              >
                {item.status === "correct"
                  ? "Correct"
